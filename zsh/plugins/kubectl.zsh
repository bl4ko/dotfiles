...
if (( ${+_comps[kubectl]} )); then
  ...
  # Define a functions `kj`,`kjx`,`ky` that runs the `kubectl` command and processes the output using different tools.
  function kj() { kubectl "$@" -o json | jq; }
  function kjx() { kubectl "$@" -o json | fx; }
  function ky() { kubectl "$@" -o yaml | yh; }

  # Setup command completions for kj,kjx,ky functions as if it were `kubectl`
  compdef kj=kubectl
  compdef kjx=kubectl
  compdef ky=kubectl
fi
